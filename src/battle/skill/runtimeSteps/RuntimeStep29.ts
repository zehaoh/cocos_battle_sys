...
export class RuntimeStep29 {
  public readonly name = 'RuntimeStep29';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 29);
    ...
    const amount = Math.max(1, Math.floor(base + 29 * 0.25));
    ...
    if (29 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 29 * 0.1, damage: amount });
    }
  }
}
