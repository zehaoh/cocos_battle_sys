...
export class RuntimeStep30 {
  public readonly name = 'RuntimeStep30';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 30);
    ...
    const amount = Math.max(1, Math.floor(base + 30 * 0.25));
    ...
    if (30 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 30 * 0.1, damage: amount });
    }
  }
}
