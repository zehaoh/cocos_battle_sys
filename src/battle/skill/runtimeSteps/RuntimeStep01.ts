...
export class RuntimeStep01 {
  public readonly name = 'RuntimeStep01';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const amount = Math.max(1, Math.floor(base + 1 * 0.25));
    ...
    if (1 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 1 * 0.1, damage: amount });
    }
  }
}
