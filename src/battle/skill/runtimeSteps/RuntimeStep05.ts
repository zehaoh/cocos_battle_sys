...
export class RuntimeStep05 {
  public readonly name = 'RuntimeStep05';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 5);
    ...
    const amount = Math.max(1, Math.floor(base + 5 * 0.25));
    ...
    if (5 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 5 * 0.1, damage: amount });
    }
  }
}
