...
export class RuntimeStep26 {
  public readonly name = 'RuntimeStep26';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 26);
    ...
    const amount = Math.max(1, Math.floor(base + 26 * 0.25));
    ...
    if (26 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 26 * 0.1, damage: amount });
    }
  }
}
