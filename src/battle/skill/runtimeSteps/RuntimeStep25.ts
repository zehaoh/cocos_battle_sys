...
export class RuntimeStep25 {
  public readonly name = 'RuntimeStep25';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 25);
    ...
    const amount = Math.max(1, Math.floor(base + 25 * 0.25));
    ...
    if (25 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 25 * 0.1, damage: amount });
    }
  }
}
