...
export class RuntimeStep04 {
  public readonly name = 'RuntimeStep04';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 4);
    ...
    const amount = Math.max(1, Math.floor(base + 4 * 0.25));
    ...
    if (4 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 4 * 0.1, damage: amount });
    }
  }
}
