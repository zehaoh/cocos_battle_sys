...
export class RuntimeStep13 {
  public readonly name = 'RuntimeStep13';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 13);
    ...
    const amount = Math.max(1, Math.floor(base + 13 * 0.25));
    ...
    if (13 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 13 * 0.1, damage: amount });
    }
  }
}
