...
export class RuntimeStep14 {
  public readonly name = 'RuntimeStep14';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 14);
    ...
    const amount = Math.max(1, Math.floor(base + 14 * 0.25));
    ...
    if (14 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 14 * 0.1, damage: amount });
    }
  }
}
