...
export class RuntimeStep17 {
  public readonly name = 'RuntimeStep17';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 17);
    ...
    const amount = Math.max(1, Math.floor(base + 17 * 0.25));
    ...
    if (17 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 17 * 0.1, damage: amount });
    }
  }
}
