...
export class RuntimeStep12 {
  public readonly name = 'RuntimeStep12';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 12);
    ...
    const amount = Math.max(1, Math.floor(base + 12 * 0.25));
    ...
    if (12 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 12 * 0.1, damage: amount });
    }
  }
}
