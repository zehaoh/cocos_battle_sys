...
export class RuntimeStep22 {
  public readonly name = 'RuntimeStep22';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 22);
    ...
    const amount = Math.max(1, Math.floor(base + 22 * 0.25));
    ...
    if (22 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 22 * 0.1, damage: amount });
    }
  }
}
