...
export class RuntimeStep21 {
  public readonly name = 'RuntimeStep21';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 21);
    ...
    const amount = Math.max(1, Math.floor(base + 21 * 0.25));
    ...
    if (21 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 21 * 0.1, damage: amount });
    }
  }
}
