...
export class RuntimeStep20 {
  public readonly name = 'RuntimeStep20';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 20);
    ...
    const amount = Math.max(1, Math.floor(base + 20 * 0.25));
    ...
    if (20 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 20 * 0.1, damage: amount });
    }
  }
}
