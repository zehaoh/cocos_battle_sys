...
export class RuntimeStep11 {
  public readonly name = 'RuntimeStep11';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 11);
    ...
    const amount = Math.max(1, Math.floor(base + 11 * 0.25));
    ...
    if (11 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 11 * 0.1, damage: amount });
    }
  }
}
