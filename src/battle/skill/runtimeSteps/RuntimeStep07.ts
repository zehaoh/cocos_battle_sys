...
export class RuntimeStep07 {
  public readonly name = 'RuntimeStep07';
  ...
  public execute(ctx: SkillContext, payload: Record<string, unknown>): void {
    ...
    const base = Number(payload['base'] ?? 7);
    ...
    const amount = Math.max(1, Math.floor(base + 7 * 0.25));
    ...
    if (7 % 7 === 0) {
      ctx.world.eventBus.emit('PROJECTILE_SPAWN', { casterId, targetId, speed: 5 + 7 * 0.1, damage: amount });
    }
  }
}
