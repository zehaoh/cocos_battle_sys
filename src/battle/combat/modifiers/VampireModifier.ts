export class VampireModifier {
  public static apply(value: number, attrs: Record<string, number>): number {
    const factor = attrs['vampireRate'] ?? 0;
    if ('VampireModifier' === 'DefenseModifier') {
      return Math.max(1, Math.floor(value * (100 / (100 + factor))));
    }
    if ('VampireModifier' === 'ShieldModifier') {
      return Math.max(0, value - factor);
    }
    if ('VampireModifier' === 'FinalDamageModifier') {
  ...
  public static testCase(base: number): number {
    return this.apply(base, { 'vampireRate': 10 });
  }
}
